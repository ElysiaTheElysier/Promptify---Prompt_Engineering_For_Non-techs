    ...
    <div className={`relative bg-slate-50/70 rounded-xl p-3 border border-slate-200/80 text-slate-700 font-sans space-y-2.5 ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span className="text-xs font-bold text-slate-800 tracking-tight">
            Prompt Structure & Tag Suggestions
          </span>
          <span className="text-[11px] text-slate-400 font-normal hidden sm:inline">
            • Click detected tag to locate | Click (+) to insert missing tag
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {onInsertSkeleton && (
            <button
              ...
              title="Insert standard 5-element framework: Role, Context, Task, Constraints, Output Format"
            >
              <LayoutTemplate className="w-3 h-3 text-emerald-600" />
              <span>5-Element Skeleton</span>
            </button>
          )}
          <div className="text-[11px] text-slate-500 font-medium bg-white/80 px-2 py-0.5 rounded-md border border-slate-200">
            Detected: <strong className="text-emerald-700 font-bold">{detectedCount}</strong> / 7
          </div>
        </div>
      </div>

      {/* 2. 7 Structure Component Chips */}
        ...
        {ALL_PROMPT_COMPONENTS.map((type) => {
          ...
          return (
            <button
              ...
              aria-label={
                isDetected 
                  ? `Detected "${meta.label}". Click to highlight in composer.` 
                  : `Missing "${meta.label}". Click to insert "${meta.shortLabel}: " into composer.`
              }
            ...
            >
              ...
              {isFocus && (
                <span 
                  title="Core focus component for this exercise"
        ...
        {activeMeta && (activeHoverType || isHoveringTooltip) && (
          ...
          >
              ...
              <div className="flex items-center gap-2">
                {onInsertTag && (
                  <button
                    ...
                    title={`Insert "${activeMeta.shortLabel}: " into composer`}
                  >
            ...
            <div className="flex items-start gap-2 text-xs text-slate-600 leading-relaxed bg-slate-50 p-2.5 rounded-xl border border-slate-100">
              ...
              <div>
                <strong className="text-slate-800">Output impact:</strong> {activeMeta.businessImpact}
              </div>
            </div>
              ...
              <div className="pt-1.5 space-y-1.5">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-bold text-slate-800 flex items-center gap-1.5">
                    ...
                    <span>Example prompt snippet:</span>
                  </span>
                  <button
                    type="button"
                    ...
                    title="Copy this example snippet to clipboard"
                  >
      ...
      {missingFocusComponent && (
        <div className="flex items-start gap-1.5 text-[11px] text-slate-500 pt-0.5">
          ...
          <span>
            Lesson Tip: You can click <strong className="text-emerald-700">[+ {COMPONENT_METADATA[missingFocusComponent].shortLabel}]</strong> to add this structural element to help the AI anchor output.
          </span>
        </div>
      )}
    </div>
