...
export const NotebookCell: React.FC<Props> = ({
  ...
  const [systemPrompt, setSystemPrompt] = useState<string>(
    lab.systemInstruction || 'You are a professional enterprise AI assistant.'
  );
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-indigo-950">Control Data (Fixed Input)</span>
                ...
                <div className="flex items-center gap-1 text-indigo-700 text-[11px] font-semibold">
                  <span>{showSampleData ? 'Collapse' : 'View Data'}</span>
              ...
              {showSampleData && (
                <div className="p-4 bg-white border-t border-indigo-100 space-y-2.5 animate-fadeIn">
                  <p className="text-[11px] text-slate-600 italic bg-indigo-50/60 p-2.5 rounded-lg border border-indigo-100/80">
                    💡 <strong>Principle:</strong> This dataset is held constant across all attempts so you can clearly observe: with the exact same input data, how changing prompt structure changes the AI output.</p>
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2.5">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${
                ...
                {runCount === 0 ? 'Not Run' : `Attempt ${runCount}`}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs">
              {runCount === 0 && !showHintOrStandardPrompt ? (
                ...
                >
                  ...
                  <span>Hints & Reference Prompt</span>
                </button>
              ) : (
                <div className="flex items-center gap-2 animate-fadeIn">
                  <button
                    ...
                    title="Load initial unrefined prompt"
                  >
                  ...
                  <button
                    ...
                    title="Load standardized prompt structure"
                  >
                    <Sparkles className="w-3 h-3 text-emerald-600" />
                    <span>Load Standard Prompt</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {showHintOrStandardPrompt && (
            <div className="p-3 bg-amber-50/70 border border-amber-200/80 rounded-xl text-xs text-amber-950 space-y-1.5 animate-fadeIn">
              <div className="font-bold flex items-center gap-1 text-amber-900">
                ...
                <span>Hints for this lesson:</span>
              </div>
          ...
          <div className="relative">
            <textarea
              ...
              placeholder="Type your prompt here (e.g., You are a specialist... Analyze...)..."
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
            <span className="text-[11px] text-slate-400 font-mono">
              {promptInput.length} chars · Press <kbd className="px-1.5 py-0.5 bg-slate-100 border border-slate-300 rounded text-slate-600 text-[10px]">Ctrl + Enter</kbd> to run
            </span>
            ...
            >
              ...
              <span>{isRunning ? 'Running prompt...' : 'Run Prompt'}</span>
            </button>
          </div>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500 font-medium">
                  (Attempt {selectedVersionNumber > 0 ? selectedVersionNumber : runCount})
                </span>
                ...
                {selectedVersionNumber > 0 && (
                  <button
                    ...
                    title="Save this version as standard prompt (SOP)"
                  >
            ...
            {scoreResult && (
              <div className="bg-white rounded-xl p-4 border border-slate-200/90 space-y-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  >
                    {showScoreDetails ? 'Collapse Rubrics' : 'View Rubrics'}
                  </button>
                </div>

                <p className="text-xs text-slate-600 leading-relaxed">
                  💡 <strong>Key Takeaway:</strong> {scoreResult.actionableAdvice}
                </p>

                {showScoreDetails && (
                  <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    <div className="p-2.5 bg-slate-50 rounded-lg">
                      <span className="font-semibold text-slate-700 block">1. Role Clarity ({scoreResult.personaScore}/20)</span>
                      <p className="text-[11px] text-slate-500 mt-0.5">{scoreResult.personaNote}</p>
                    </div>
                    <div className="p-2.5 bg-slate-50 rounded-lg">
                      <span className="font-semibold text-slate-700 block">2. Task Specificity ({scoreResult.taskScore}/20)</span>
                      <p className="text-[11px] text-slate-500 mt-0.5">{scoreResult.taskNote}</p>
                    </div>
                    <div className="p-2.5 bg-slate-50 rounded-lg">
                      <span className="font-semibold text-slate-700 block">3. Safety Guardrails ({scoreResult.guardrailsScore}/20)</span>
                      <p className="text-[11px] text-slate-500 mt-0.5">{scoreResult.guardrailsNote}</p>
                    </div>
                    <div className="p-2.5 bg-slate-50 rounded-lg">
                      <span className="font-semibold text-slate-700 block">4. Output Format ({scoreResult.formatScore}/20)</span>
